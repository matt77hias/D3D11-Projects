...
Renderer::Renderer(HWND hwindow) : m_loaded(false), m_hwindow(hwindow), m_render_target_view(nullptr), m_swap_chain2 (nullptr), m_device_context2(nullptr), m_device2(nullptr),
		m_vertex_shader(nullptr), m_pixel_shader(nullptr), m_vertex_layout(nullptr), m_vertex_buffer(nullptr), m_index_buffer(nullptr),
		m_depth_stencil(nullptr), m_depth_stencil_view(nullptr),
		m_cb_never_changes(nullptr), m_cb_change_on_resize(nullptr), m_cb_changes_every_frame(nullptr),
		m_texture_resource_view(nullptr), m_sampler_linear(nullptr),
...
HRESULT Renderer::InitScene() {
	...
	// Constant buffers setup
	{
		...
		// 2. A pointer to a D3D11_SUBRESOURCE_DATA structure that describes the initialization data.
		// 3. Address of a pointer to the ID3D11Buffer interface for the buffer object created.
		const HRESULT result_cb_never_changes = m_device2->CreateBuffer(&buffer_desc, nullptr, &m_cb_never_changes);
		...
		// 2. A pointer to a D3D11_SUBRESOURCE_DATA structure that describes the initialization data.
		// 3. Address of a pointer to the ID3D11Buffer interface for the buffer object created.
		const HRESULT result_cb_change_on_resize = m_device2->CreateBuffer(&buffer_desc, nullptr, &m_cb_change_on_resize);
		...
		// 2. A pointer to a D3D11_SUBRESOURCE_DATA structure that describes the initialization data.
		// 3. Address of a pointer to the ID3D11Buffer interface for the buffer object created.
		const HRESULT result_cb_changes_every_frame = m_device2->CreateBuffer(&buffer_desc, nullptr, &m_cb_changes_every_frame);
	...
	{
		// Load the texture.
		const HRESULT result_texture = CreateDDSTextureFromFile(m_device2, L"seafloor.dds", nullptr, &m_texture_resource_view);
...
HRESULT Renderer::InitCamera() {
	...
	m_device_context2->UpdateSubresource(m_cb_never_changes, 0, nullptr, &buffer, 0, 0);
	...
	m_device_context2->UpdateSubresource(m_cb_change_on_resize, 0, nullptr, &buffer2, 0, 0);
...
void Renderer::Render(double elapsed_time) {
	...
	m_device_context2->UpdateSubresource(m_cb_changes_every_frame, 0, nullptr, &buffer, 0, 0);
