...
Renderer::Renderer(HWND hwindow) : m_loaded(false), m_hwindow(hwindow), m_render_target_view(nullptr), m_swap_chain2 (nullptr), m_device_context2(nullptr), m_device2(nullptr),
		m_vertex_shader(nullptr), m_pixel_shader(nullptr), m_vertex_layout(nullptr), m_vertex_buffer(nullptr), m_index_buffer(nullptr), m_constant_buffer(nullptr),
...
HRESULT Renderer::InitDevice() {
	...
	const HRESULT result_vertex_shader_blob = CompileShaderFromFile(L"Project 5/shaders/effect.fx", "VS", "vs_4_0", &vertex_shader_blob);
#else
...
#ifdef ENGINE_COMPILE_SHADERS
	const HRESULT result_pixel_shader_blob = CompileShaderFromFile(L"Project 5/shaders/effect.fx", "PS", "ps_4_0", &pixel_shader_blob);
#else
...
void Renderer::Render(double elapsed_time) {
	...
	m_device_context2->UpdateSubresource(m_constant_buffer, 0, nullptr, &buffer2, 0, 0);
	m_device_context2->DrawIndexed(36, 0, 0);
