...
namespace {
	...
	class MessageHandler : public WindowMessageHandler {
	...
	public:
		...
		virtual const std::optional< LRESULT >
			...
			if (WM_SYSKEYDOWN == message && VK_RETURN == wParam) {
				g_mode_switch = true;
				return 0;
			}
	...
	MessageHandler g_message_handler;
	...
	void Init(NotNull< HINSTANCE > instance) {
		...
		// Initialize a window.
		{
			...
			g_window->AddHandler(NotNull< WindowMessageHandler* >(&g_message_handler));
		}
	...
	[[nodiscard]]
	bool LostMode() noexcept {
		BOOL current = FALSE;
		g_swap_chain->GetFullscreenState(&current, nullptr);
		const auto fullscreen = (FALSE != current);
		return fullscreen != g_fullscreen;
	}
	...
	void SwitchMode(bool toggle) {
		...
		if (toggle) {
			g_swap_chain->GetFullscreenState(&current, nullptr);
			current = !current;
			g_swap_chain->SetFullscreenState(current, nullptr);
		}

		// Recreate the swap chain buffers.
		g_swap_chain->ResizeBuffers(0u, 0u, 0u,
									DXGI_FORMAT_UNKNOWN,
	...
	[[nodiscard]]
	int Run(int nCmdShow, bool start_fullscreen) {
		...
		while (WM_QUIT != msg.message) {
			...
			const auto lost_mode = LostMode();
			if (g_mode_switch || lost_mode) {
				SwitchMode(!lost_mode);
				g_mode_switch = false;
				continue;
			}
