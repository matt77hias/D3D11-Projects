...
namespace {
	...
	void Init(NotNull< HINSTANCE > instance) {
		...
		// Initialize the camera.
		{
			...
			static const XMVECTOR p_eye   = { 0.0f, 1.0f, -5.0f, 0.0f };
	...
	void Render(F64 time) {
		...
		// Animate the cube.
		{
			...
			g_camera.m_object_to_world = XMMatrixRotationY(5.0f * static_cast< F32 >(time));

			Transform transform;
			transform.m_object_to_world      = XMMatrixTranspose(g_camera.m_object_to_world);
